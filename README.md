...
A Medical Data eXchange (MedX) platform based on the Ethereum blockchain
...
async function main() {
  ...
  */
  ...
  /*
    ...
    STEP6: revoke a new EAS
           enter the directory ID from STEP1, user type, 
           user (provider or consumer) ID and password from STEP2 or STEP3
  ...
  */  
  ...
  try {
    options = {
      url: 'https://dxdl.deepq.com:5000/entry/dctf/' + '?directoryID=' + directoryID
                                                     + '&dataCertificate=' + '49b45u9gm9042mg04b24jbj2=bj2=bj',
    };
    await request.get(options, (error, res, body) => {
      result = JSON.parse(res.body).result;
